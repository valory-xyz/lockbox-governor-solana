...
async function main() {
  ...
  const posted = new anchor.web3.PublicKey("GMrUcHq2vhZQDc2npr2AwBbmmQUgyJ3RfHAz9W34kmLX");
  ...
  const vaaHashTransferAll = Buffer.from([
     19,  37, 253,  76, 240, 142,  43, 161,
    131, 235, 108,  37,  12,  12,  92, 216,
    171, 253, 198,  74,  10, 176,  81, 103,
    232, 229, 199, 132,  95, 251, 186, 178
  ]);
  ...
  console.log("timelock", timelock.toBase58());
    ...
    //console.log("OLAS ATA in hex", bs58.decode(tokenOwnerAccountB.address.toBase58()).toString("hex"));
  ...
  let signature = null;
    ...
    try {
        signature = await program.methods
          .transferAll(vaaHashTransferAll)
          .accounts(
            {
              config: pdaConfig,
              ...
              destinationAccountSol: tokenOwnerAccountA.address,
              destinationAccountOlas: tokenOwnerAccountB.address
            }
          )
          .rpc();
    } catch (error) {
    ...
    console.log("Successfully transferred all the funds");
}
